...
for(i in 1:10){
  ...
  # #   
  #  pmt <- UniProbitTree(min_leaf_size=20,max_depth=1,M_max=200)
  # 
  #    pmt$fit(x=xtrain,y=ytrain,n=length(ytrain),prune_tree =FALSE,aic=FALSE)
  #   pred <- pmt$predict(xtest)
  #   confmatrix <- table(pred,ytest)
  # 
  ...
  #   print(cvpbt[i])
    ...
    # re <- PMT(xtrain,ytrain,w=rep(1/length(ytrain),length(ytrain)),depth=5,min_size=20,M=10)
    # pmt.predict(re,xtest)
    # 
    #       
    #   w_init <- rep(1/length(ytrain),length(ytrain))
    #   
    #   tpx_init <- numeric(nrow(xtest))
    #  a <- WoProbitBoost(xtrain, ytrain, Wo=w_init, M_max = 20)
    # AdaPMT(ms=10,M=5,depth=5,xtrain,ytrain,xtest,step=1,w_init,tpx_init,size=15,lab_list= as.vector(sort(unique(ytrain))))
    # 
    # 
sbpmt_pred <- SBPMT(n_tree=50,n_iteration=1,M=200,depth=6,xtrain=xtrain,ytrain=ytrain,xtest=xtest,step=0.5,size=20,alpha=0.7)
  ...
  print(cvbpbtrf[i])
    ...
    tree <- rpart(ytrain~., data = xtrain)
    p <-ifelse(predict(tree, xtest)>0.5,1,0) #predict(tree, xtest,type = 'class') #ifelse(predict(tree, xtest,type = 'class')>0.5,1,0)
    confmatrix_tree <-table(p, ytest)
    acct <- (confmatrix_tree[1,1]+confmatrix_tree[2,2])/( sum(confmatrix_tree))
    cvdt[i] <-acct
    print(cvdt[i])
    #gbm
    ...
    confmatrix_tree_lmt <- table(ytest,predict(lmt,xtest))
    acct <- (sum(diag(confmatrix_tree_lmt)))/( sum(confmatrix_tree_lmt))
    clmt[i] <- acct
    print( clmt[i])
...
cat("\nmean prediction accuracy of boosted pmt random forest:",mean(cvbpbtrf)*100) # CART
cat("\nmean prediction accuracy of DT:",mean(cvdt)*100)
cat("\nmean prediction accuracy of RF:",mean(cvrf)*100)
cat("\nmean prediction accuracy of gradient boosting:",mean(cvgbm)*100)
cat("\nmean prediction accuracy of PMT:",mean(cvpbt)*100)
cat("\nmean prediction accuracy of LMT:",mean(clmt)*100)
...
cat("\nstd prediction accuracy of boosted pmt random forest:",sd(cvbpbtrf)*100) # CART
cat("\nstd prediction accuracy of DT:",sd(cvdt)*100)
cat("\nstd prediction accuracy of RF:",sd(cvrf)*100)
cat("\nstd prediction accuracy of PMT:",sd(cvpbt)*100)
cat("\nstd prediction accuracy of gradient boosting:",sd(cvgbm)*100)
cat("\nstd prediction accuracy of LMT:",sd(clmt)*100)
...
sbpmt_pred <- SBPMT(n_tree=21,n_iteration=5,M=200,depth=6,xtrain=xtrain,ytrain=ytrain,xtest=xtest,step=0.5,size=20,alpha=0.7)
  ...
  print(cvbpbtrf[i])
       ...
       tree <- rpart(ytrain~., data = xtrain)
    p <- predict(tree, xtest,type='class')#ifelse(predict(tree, xtest)>0.5,1,0) #predict(tree, xtest,type = 'class') #ifelse(predict(tree, xtest,type = 'class')>0.5,1,0)
    confmatrix_tree <-table(p, ytest)
    acct <- (sum(diag(confmatrix_tree)))/( sum(confmatrix_tree))
    cvdt[i] <-acct
